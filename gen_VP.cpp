...
void gen_VP(void *snodes_in, void *VFN_3d_1, void *VFN_3d_2 ){
					...
					for (i_s = 0; i_s < n_s; i_s++) {
						...
						for (w_i = 0; w_i < w_n; w_i++) {
							for (t_i2 = 0; t_i2 < t_n; t_i2++) {
								...
								// load previous w_i policy as a benchmark
								// MODS HERE
								(*rr1).get_pol(t_i, i_rm, i_am, i_dm, i_s, w_i - 1, x_lag_w);
								t_i2_lag_w = (*rr1).xt_grid[t_i][i_rm][i_am][i_dm][i_s][max(w_i - 1, 0)];
								v_lag_w = (*rr1).vw3_grid[t_i][i_rm][i_am][i_dm][i_s][max(w_i - 1, 0)];

								// load value given previous t_i as a benchmark
								v_lag_t = (*rr1).vw3_grid[t_i][i_rm][i_am][i_dm][i_s][w_i];
								...
								if ((v1 > v_lag_t) || (t_i2 == 0)) {                      // current result is better than value given previous tenure
									...
									// MOD HERE
									(*rr1).set_pol_ten_v(t_i, i_rm, i_am, i_dm, i_s, w_i, x1, t_i2, v1);  // set x, t_i2, v0 in

									if (res1.valid_flag == 0) {
										//(*rr1).set_pol_ten_v(t_i, i_s, w_i, x1, 0, v1);
										// MOD HERE
										(*rr1).set_pol_ten_v(t_i, i_rm, i_am, i_dm, i_s, w_i, x1, 0, v1);
									}
								}
							}
						}
						...
						// MOD HERE
						(*rr1).interp_vw3(t_i, i_rm, i_am, i_dm, i_s);  // clean and interpolate grid	
	...
	for (t_i = 1; t_i < t_n; t_i++) {                        // consider t_i = 0 first; case: begin with renter 
		for (i_rm = 0; i_rm < rm_n; i_rm++) {                // MODS HERE
			for (i_am = 0; i_am < am_n; i_am++) {            // MODS HERE
				for (i_dm = 0; i_dm < dm_n; i_dm++) {            // MODS HERE
					for (i_s = 0; i_s < n_s; i_s++) {
						i_yi = (*snodes1).s2i_yi[i_s];
						i_rent = (*snodes1).s2i_rent[i_s];
						i_ph = (*snodes1).s2i_ph[i_s];
						...
						for (w_i = 0; w_i < w_n; w_i++) {
							...
							// load previous w_i policy as an initial guess
							(*rr1).get_pol(t_i, i_rm, i_am, i_dm, i_s, w_i - 1, x_lag_w);                       // get x pol sol from previous w_i and assign to x
							t_i2_lag_w = (*rr1).xt_grid[t_i][i_rm][i_am][i_dm][i_s][max(w_i - 1, 0)];
							v_lag_w = (*rr1).vw3_grid[t_i][i_rm][i_am][i_dm][i_s][max(w_i - 1, 0)];
							...
							// MOD HERE:
							(*rr1).set_pol_ten_v(t_i, i_rm, i_am, i_dm, i_s, w_i, x1, t_i2, v1);
							...
							// MOD HERE
							res_t_0 = (*rr1).eval_v(0, i_rm, i_am, i_dm, i_s, w_adj); // evaluate value fn if agent sells and converts to renter
							...
							if ((w_adj >= 0.0) && (res_t_0.v_i_floor > v1) && (res_t_0.w_i_floor >= 0)) {
								...
								// MODS HERE
								(*rr1).get_pol(0, i_rm, i_am, i_dm, i_s, res_t_0.w_i_floor, x);                         // submit x as reference and load in x pol from t1 = 0
								t_adj = (*rr1).xt_grid[0][i_rm][i_am][i_dm][i_s][res_t_0.w_i_floor];                    // get t2 pol from t1 = 0; simulated sale
								(*rr1).set_pol_ten_v(t_i, i_rm, i_am, i_dm, i_s, w_i, x, t_adj, res_t_0.v_i_floor);     // first arguments are current state variables, x containts updated policy
						...
						// MOD HERE
						(*rr1).interp_vw3(t_i, i_rm, i_am, i_dm, i_s);  // clean grid
			...
			for (i_am = 0; i_am < am_n; i_am++) {
				for (i_dm = 0; i_dm < dm_n; i_dm++) {
					for (i_s = 0; i_s < n_s; i_s++) {
						...
						(*rr1).interp_vw3(t_i, i_rm, i_am, i_dm, i_s);  // clean grid; MOD HERE
					}
				}
			}
		}
